...
<body>
    <div class="container">
        ...
        <div class="container p-2  mt-4">
            <div>user name:{{ $message->name }}</div>
            <div>user email:{{ $message->email }}</div>
            <div>user phone:{{ $message->phone }}</div>
            <div>message:{{ $message->message }}</div>
            <div class="mt-3">
                <a class="btn btn-info" href="{{ route('admin.message_replay',$message->id) }}">replay</a>
                <form class="d-inline" action="{{ route('admin.message_destroy',$message->id) }}" method="POST">
                    @csrf
                    @method('delete')
